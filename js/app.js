...
function render(tasks, filters) {
    ...
    if (tasks.length === 0) {
        ...
        emptyText.textContent = "Add a new follow-up task to get started.";
        emptyState.hidden = false;
    } else if (filtered.length === 0) {
        emptyTitle.textContent = "No tasks match your filters";
        emptyText.textContent = "Try changing the filters to see tasks.";
        emptyState.hidden = false;
    } else {
    ...
    for (const task of filtered) {
        ...
        caseTh.scope = "row";
        caseTh.textContent = task.caseId;
        ...
        noteTd.className = "cell-note";
        noteTd.textContent = task.note;

        const createdTd = document.createElement("td");
        createdTd.className = "muted cell-created";
        ...
        tr.append(statusTd, caseTh, prioTd, noteTd, createdTd, actionsTd);
        tbody.appendChild(tr);
    }

    summary.textContent = `Status: ${filters.statusFilter}; Priority: ${filters.priorityFilter}; Showing ${filtered.length} of ${tasks.length} tasks`;
}
...
function main() {
    ...
    const caseInput = byId("caseId");
    const noteInput = byId("note");
    const tbody = byId("taskTableBody");
    const tasksRegion = document.getElementById("tasksRegion");
    ...
    form.addEventListener("submit", (event) => {
        ...
        const caseId = safeTrim(caseInput.value);
        const priority = byId("priority").value;
        const note = safeTrim(noteInput.value);

        if (!caseId) {
            caseInput.setCustomValidity("Please enter a case ID.");
            caseInput.reportValidity();
            caseInput.focus();
            return;
        }

        if (!note) {
            noteInput.setCustomValidity("Please enter an internal note.");
    ...
    tbody.addEventListener("click", (event) => {
        ...
        if (action === "delete") {
            ...
            const caseRef = existing?.caseId ? `case ${existing.caseId}` : "this case";
            const ok = confirm(`Delete this follow-up task for ${caseRef}? This can’t be undone.`);
            if (!ok) return;

            tasks = tasks.filter((t) => t.id !== taskId);
            refresh();

            if (tasksRegion && typeof tasksRegion.focus === "function") {
                tasksRegion.focus();
            }
            return;
        }
